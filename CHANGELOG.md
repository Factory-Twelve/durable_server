...
- Introduce the explicitly discriminated stored-state envelope used by the hardened EKV backend. This format is intentionally not mixed-version compatible with 0.1.4 EKV records: first activation must use an empty prefix/namespace, and existing EKV installations require a full writer drain plus offline migration or a cutover to a fresh prefix before any 0.1.5 writer starts. Do not perform a rolling deployment with old and new EKV writers/readers overlapping.
